...
class Q_criterion(Gradient):
    ...
    def main_proc(self):
        ...
        if rank == 0:
            ele_wall = self._get_eles()
        else:
            ele_wall = None

        ele_wall = comm.bcast(ele_wall, root=0)
        """
        plt.figure()
        for k, v in ele_wall.items():
            print(k)
        ...
        #plt.show()
        """
        ele_wall = self._restrict_domain(ele_wall)

        mesh = self._pre_proc_mesh_local(ele_wall)

        # Get time series
        time = self.get_time_series_mpi(rank, size)
        for t in time:
            print(t)
            self._proc_soln(t, ele_wall, mesh)

    def _pre_proc_mesh_local(self, mesh_wall, op = False):
        mesh = {}
    ...
    def _proc_soln(self, time, mesh_wall, mesh):
        soln = f'{self.solndir}{time}.pyfrs'
        soln = self._load_snapshot(soln, mesh_wall)

        Q = {}
        for etype, sln in soln.items():
            Q[etype] = self._proc_Q(etype, mesh[etype], sln)
            #self._proc_Q(etype, mesh[etype], sln)
        #for etype in soln:
    ...
    def _flash_to_vtk(self, mesh, Q, time):
        ...
        for pfn, misil in parts.items():
            with open(pfn, 'wb') as fh:
                ...
                # Header
                for pn, k in misil:
                    off = self._write_serial_header(fh, Q[k], off)
    ...
    def _write_data(self, vtuf, pn, mk, sk, name):
        ...
        # Perform the sub division for high order output
        ...
        #print(vtu_con)
        #print(vtu_off)
        #print(vtu_con.shape, vtu_off.shape, vtu_typ.shape, vpts.swapaxes(0, 1).shape, soln.T[...,None].shape)
    ...
    def _write_serial_header(self, vtuf, sk, off):
        ...
        write_s(f'<Piece NumberOfPoints="{npts*ncells}" NumberOfCells="{ncells}">\n'
                '<Points>\n')
